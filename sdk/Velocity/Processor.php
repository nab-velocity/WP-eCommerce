...
class VelocityProcessor {
    ...
    public function verify($options = array()) {
        ...
        try {
            ...
            list($error, $response) = $this->connection->post(
                    $this->path(
                            self::$workflowid, self::$Txn_method[0], self::$Txn_method[0]
                    ), array(
    ...
    public function authorizeAndCapture($options = array()) {
        ...
        try {
            ...
            list($error, $response) = $this->connection->post(
                    $this->path(
                            self::$workflowid, null, self::$Txn_method[2]
                    ), array(
                'sessiontoken' => $this->sessionToken,
                'xml' => $body,
    ...
    public function authorize($options = array()) {
        ...
        try {
            ...
            list($error, $response) = $this->connection->post(
                    $this->path(
                            self::$workflowid, null, self::$Txn_method[1]
                    ), array(
                'sessiontoken' => $this->sessionToken,
                'xml' => $body,
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            ...
            try {
                ...
                list($error, $response) = $this->connection->put(
                        $this->path(
                                self::$workflowid, $options['TransactionId'], self::$Txn_method[3]
                        ), array(
                    'sessiontoken' => $this->sessionToken,
                    'xml' => $body,
    ...
    public function adjust($options = array()) {
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            ...
            try {
                ...
                list($error, $response) = $this->connection->put(
                        $this->path(
                                self::$workflowid, $options['TransactionId'], self::$Txn_method[4]
                        ), array(
                    'sessiontoken' => $this->sessionToken,
                    'xml' => $body,
    ...
    public function undo($options = array()) {
        ...
        if (isset($options['TransactionId'])) {
            ...
            try {
                ...
                list($error, $response) = $this->connection->put(
                        $this->path(
                                self::$workflowid, $options['TransactionId'], self::$Txn_method[5]
                        ), array(
                    'sessiontoken' => $this->sessionToken,
                    'xml' => $body,
    ...
    public function returnById($options = array()) {
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            ...
            try {
                ...
                list($error, $response) = $this->connection->post(
                        $this->path(
                                self::$workflowid, null, self::$Txn_method[6]
                        ), array(
                    'sessiontoken' => $this->sessionToken,
                    'xml' => $body,
    ...
    public function returnUnlinked($options = array()) {
        ...
        try {
            ...
            list($error, $response) = $this->connection->post(
                    $this->path(
                            self::$workflowid, null, self::$Txn_method[7]
                    ), array(
                'sessiontoken' => $this->sessionToken,
                'xml' => $body,
    ...
    public function queryTransactionsDetail($guerytxndetail) {
        ...
        try {
            ...
            list($error, $response) = $this->connection->post('DataServices/TMS/transactionsDetail', array(
                'sessiontoken' => $this->sessionToken,
                'xml' => $body,
